...
- `admin-staff.html`: 임직원을 카드 단위로 추가·삭제·순서변경하고, 사진·이름·성향(MBTI)·근무형태(강사/사무실/셔틀)·강의종별(학과교육/기능교육/도로주행/대형·특수/도로연수, 복수 선택 가능)을 입력해 저장합니다. "근무형태=강사 또는 사무실"인 인원만 `about.html` 하단의 강사진 캐러셀 대상이 되고(셔틀 인력은 내부 명단 관리용), 그 중에서도 방문자가 "강사진"/"사무실" 탭으로 골라서 봅니다. 사진은 `api/upload.php`로 업로드되어 `uploads/staff/`에 저장됩니다. 몇 명을 등록하든 개수 제한은 없습니다 — 인원이 많아지면(성수기 강사 충원 등) 캐러셀이 자동으로 사진 크기와 배치 반지름을 줄여 겹치지 않게 배치합니다(4명까지 150px, 이후 서서히 축소되어 최소 70px까지).
  - **about.html 캐러셀 사용법**: "강사진"/"사무실" 탭을 눌러 조직을 고르면 해당 인원만 캐러셀에 나타납니다(선택한 조직에 등록된 인원이 없으면 안내 문구만 표시). 원형 사진을 마우스/터치로 좌우 드래그하면 3D로 회전하며 다음 인물로 전환됩니다. 점(dot) 클릭이나 캐러셀에 포커스 후 방향키(←/→)로도 이동할 수 있고, 아래에 이름·강의종별·MBTI가 표시됩니다. 등록된 강사·사무실 인원이 전혀 없거나 DB 연동 전(정적 호스팅)에는 탭 없이 예시 팀 4개로 자동 대체됩니다.
